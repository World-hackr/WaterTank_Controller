...
void loop() {
  ...
  // 2. Listen for Diagnostic Packets
  uint8_t packet[8];
  uint8_t len = sizeof(packet);
  if (radio_recv(packet, &len)) {
    if (len == 8 && packet[0] == 0xAA) {
      // Diagnostic Packet Detected!
      rxButton = packet[1]; // Button status
      rxLevel = packet[2];  // Level status
      
      lastPacketTimeMs = now;
      showHeartbeat = true;
      heartbeatLedOffMs = now + 100; // Blink LED 6 for 100ms
    }
  }

  // 3. Relay Action: Relay turns ON when Transmitter button is pressed
  if (rxButton == 0 && (now - lastPacketTimeMs < 3000)) {
    PORTA.OUTSET = RELAY_PIN_bm; // Pump ON
  } else {
    PORTA.OUTCLR = RELAY_PIN_bm; // Pump OFF
  }

  // 4. LED Diagnostic Display Scan
  // LED 6 = Heartbeat flash when packet arrives
  if (showHeartbeat && (now < heartbeatLedOffMs)) {
    drive_led(6);
    _delay_ms(2);
  } else {
    showHeartbeat = false;
  ...
  // LED 5 = Shows state of Transmitter Button (ON if button pressed)
  if (rxButton == 0 && (now - lastPacketTimeMs < 3000)) {
    drive_led(5);
    _delay_ms(2);
  }

  // LEDs 1 to 4 = Show Water Probe Level (Solid level indicators)
  if (now - lastPacketTimeMs < 4000) { // Keep display active for 4 seconds after last packet
    uint8_t level = rxLevel > 4 ? 4 : rxLevel;
