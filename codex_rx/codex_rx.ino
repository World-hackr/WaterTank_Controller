...
static void xtea_decrypt(uint32_t num_rounds, uint8_t ciphertext[8], uint32_t const k[4]) {
  uint32_t v[2];
  for (uint8_t i = 0; i < 8; i++) ((uint8_t*)v)[i] = ciphertext[i];
  ...
  v[0] = v0; v[1] = v1;
  for (uint8_t i = 0; i < 8; i++) ciphertext[i] = ((uint8_t*)v)[i];
}
...
void loop() {
  ...
  if (radio_recv(ciphertext, &len)) {
    if (len == 8) {
      ...
      xtea_decrypt(32, ciphertext, activeKey);
